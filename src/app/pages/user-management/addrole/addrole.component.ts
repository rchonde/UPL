...
import { FormControl, FormGroupDirective, UntypedFormBuilder, UntypedFormGroup, NgForm, Validators, UntypedFormArray } from '@angular/forms';
...
_pageRights:any;
  ...
  get roles() { return this.AddRoleForm.get('Roles') as UntypedFormArray; }
  get _PageRight() { return this.AddRoleForm.get('_PageRight') as UntypedFormArray; }
  ...
  getPageList(TID:number) {  
    ...
    OnSelectAll() {
      ...
      this.roles.controls.forEach((role) => {
        role.patchValue({ isChecked: _bool });
        let subItems = role.get("subItems") as UntypedFormArray;
